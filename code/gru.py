...
class GRU(GRUAbstract):
    ...
    def forward(self, x, s_previous):
        ...
        '''
        #Initialize the input one-hot vector x
        #Note s_previous has shape (hidden_dims,)
        x_t = np.zeros(self.vocab_size)
        x_t[x] = 1

        #DELETE FOR FINAL VERSION: Need to double check the first time it is called it is the right shape (will assume this for now)

        r = sigmoid(self.Vr @ x_t + self.Ur @ s_previous)  # Update gate
        z = sigmoid(self.Vz @ x_t + self.Uz @ s_previous)  # Reset gate
        h = np.tanh(self.Vh @ x_t + self.Uh @ (r * s_previous))  # Candidate activation
        s = z * s_previous + (1-z) * h  # New hidden state
        net_t_out = self.W @ s  # Output layer linear transformation
        y = softmax(net_t_out)  # Output layer activation
    ...
    def acc_deltas_np(self, x, d, y, s):
        ...
        current_d = np.zeros(self.out_vocab_size)
    ...
    def acc_deltas_bptt_np(self, x, d, y, s, steps):
        ...
        #Question: don't think there is any difference between our implementation of acc deltas and acc deltas bptt
        t = len(x)-1
