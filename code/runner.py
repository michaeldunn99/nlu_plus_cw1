...
class Runner(object):
    ...
    def compute_loss(self, x, d):
        ...
        prediction_probability_matrix = self.model.predict(x)[0]
        indices = np.array(d)
    ...
    def compute_loss_np(self, x, d):
        ...
        prediction_probability_vector = self.model.predict(x)[0][-1,:]
        selected_probability = prediction_probability_vector[d[0]]
        loss = -np.log(selected_probability)
    ...
    def compute_acc_np(self, x, d):
        ...
        actual_prediction_probability_vector = self.model.predict(x)[0][-1,:]
        if np.argmax(actual_prediction_probability_vector) == d[0]:
            return 1
        else:
            return 0
    ...
    def compute_mean_loss(self, X, D):
        ...
        total_length = np.sum([len(seq) for seq in X])
        # compute_loss_vectorized = np.vectorize(self.compute_loss, signature='(n),(m)->()')
        total_loss = 0
...
if __name__ == "__main__":
    ...
    if mode == "train-lm-rnn":
        ...
        annealing_rate = float(sys.argv[7])
        print(f"\n\nRunning for hyperparameter settings:\n\thdim = {hdim}\n\tlookback = {lookback}\n\tlearning rate = {lr}")
        ...
        #In this case the input vocab size and output vocab size are the same
        my_rnn = RNN(vocab_size, hdim, vocab_size)
        my_runner = Runner(my_rnn)
        my_runner.train(X_train, D_train, X_dev, D_dev,learning_rate = lr, back_steps = lookback, anneal=annealing_rate, epochs=epochs)
        current_hyper_params = f"hdim{hdim}_lookback{lookback}_lr{lr}_ar{annealing_rate}_epochs{epochs}"
        os.makedirs(f"parameters/{current_hyper_params}", exist_ok=True)
        np.save(f"parameters/{current_hyper_params}/W", my_runner.model.W)
        np.save(f"parameters/{current_hyper_params}/U", my_runner.model.U)
        np.save(f"parameters/{current_hyper_params}/V", my_runner.model.V)

        #To ask about this stuff
        
        run_loss = -1
        mean_loss_dev = my_runner.compute_mean_loss(X_dev, D_dev)
        print(f"Run_loss on dev_set = {mean_loss_dev}")
        adjusted_loss = -1
    ...
    if mode == "train-np-rnn":
        ...
        #In this case the output vocab size is 2
        output_vocab_size = 2
        my_np_rnn = RNN(vocab_size, hdim, output_vocab_size)
        my_np_runner = Runner(my_np_rnn)
        print("Training RNN np: ")
        my_np_runner.train_np(X_train, D_train, X_dev, D_dev,learning_rate = lr, back_steps = lookback)


        acc = sum([my_np_runner.compute_acc_np(X_dev[i], D_dev[i]) for i in range(len(X_dev))]) / len(X_dev)
    ...
    if mode == "train-np-gru":
        ...
        my_gru_runner = Runner(my_np_gru)
        print("Training GRU np: ")
        my_gru_runner.train_np(X_train, D_train, X_dev, D_dev,learning_rate = lr, back_steps = lookback)

        acc = sum([my_gru_runner.compute_acc_np(X_dev[i], D_dev[i]) for i in range(len(X_dev))]) / len(X_dev)
