...
if __name__ == "__main__":
    ...
    if mode == "train-lm-rnn":
        ...
        my_runner.train(X_train, D_train, X_dev, D_dev,learning_rate = lr, back_steps = lookback)
        current_hyper_params = f"hdim{hdim}_lookback{lookback}_lr{lr}"
        os.makedirs(f"parameters/best_params_25k/{current_hyper_params}", exist_ok=True)
        np.save(f"parameters/best_params_25k/{current_hyper_params}/W", my_runner.model.W)
        np.save(f"parameters/best_params_25k/{current_hyper_params}/U", my_runner.model.U)
        np.save(f"parameters/best_params_25k/{current_hyper_params}/V", my_runner.model.V)
